class Admin::BoxesController < ApplicationController
  ...
  def create
    @admin_box = Admin::Box.new(admin_box_params)

    respond_to do |format|
      if @admin_box.save
        format.html { redirect_to @admin_box, notice: 'Box was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @admin_box.update(admin_box_params)
        format.html { redirect_to @admin_box, notice: 'Box was successfully updated.' }
  ...
  private
    ...
    def set_admin_box
      @admin_box = Admin::Box.find(params[:id])
    end
